【安卓】远航工具：https://fy6b.lanzouq.com/iAP0K1y29gle

【安卓】一个工坊：https://fy6b.lanzouq.com/ieO8W1wlhrgf

【Win】路遥工具箱：https://luyao.coderbusy.com/LuYao.Toolkit.zip

【Win】三个工具合集：https://share.feijipan.com/s/uMA62Chn

【Win】JamTools：https://pan.lanzouo.com/iXuT51s4y0kd

【Win】侠客视频下载：https://fy6b.lanzouq.com/iM1BA1ss73jc

【Win】两款配音工具：https://fy6b.lanzouq.com/b02ly911e

分享9款精品学习软件

1：直链下载：https://muziqi.lanzoul.com/b00s7yhfc
密码：5kx7

2：汉语字典和成语词典：https://info.appstore.vivo.com.cn/detail/3538935

3：棋路：https://info.appstore.vivo.com.cn/detail/2959493

4：百度云链接：https://pan.baidu.com/s/1h_2V3RcbIjOSVB9ngF3XXw 
提取码：86fv 

   

- 奶牛快传（在线传输文件利器）：cowtransfer.com
- 文叔叔（大文件传输，不限速）：https://www.wenshushu.cn/
- 云端超级应用空间（PS，PPT，Excel，Ai）：https://uzer.me/
- 香当网（年终总结，个人简历，事迹材料，租赁合同，演讲稿）：https://www.xiangdang.net/
- 二维码生成：https://cli.im/
- 搜狗翻译：fanyi.sogou.com
- 熵数（图表制作，数据可视化）：https://dydata.io/appv2/#/pages/index/home
- 拷贝兔：https://cp.anyknew.com/
- 图片无限变放大：http://bigjpg.com/zh
- 幕布（在线大纲笔记工具）：mubu.com
- 在线转换器（在线转换器转换任何测量单位）：https://zh.justcnw.com/
- 调查问卷制作：https://www.wenjuan.com/
- 果核剥壳（软件下载）：https://www.ghpym.com/
- 软件下载：https://www.unyoo.com/
- MSDN我告诉你（windows10系统镜像下载）：https://msdn.itellyou.cn/
- 傲梅备份：https://www.abackup.com/
- DeepL：https://www.deepl.com/translator
- Deskreen，电脑投屏工具，可以通过 WiFi 将任何带有网络浏览器的设备变成电脑的辅助屏幕：https://deskreen.com/
- TreeSize，磁盘分析工具，它可以非常方便的查看文件和目录大小：https://www.jam-software.com/treesize_free
- CrossOver，系统兼容软件：http://www.crossoverchina.com

AnLink：https://cn.anlinksoft.com/

> 一个完全免费的投屏工具，既可以让电脑控制手机，也能够让手机投屏到电脑。支持WiFi和USB连接、多点触控、共享剪贴板等功能。

TTSMaker：https://ttsmaker.cn/

> 一款免费的文本转语音工具，提供语音合成服务。支持多种语言，包括中文、英语、日语、韩语、法语、德语、西班牙语、阿拉伯语等50多种语言，以及超过300种语音风格。

### Trello

https://trello.com

Trello 是**一个免费的项目管理软件**，它采用卡片式的界面进行任务管理。

用户可以在Trello上创建看板(Board)，列表(List)，创建卡片(Card)。

### 聊天记录生成器

*https://weixinduihua2.com/*

一个非常好玩的娱乐软件，可以生成微信或支付宝对话，现在很多搞笑短视频就是利用这种软件在自我创作。



1. Youtubes视频下载：https://quanwang.su27.net

2. YouTube视频下载平台2：https://youtube.iiilab.com/

3. 视频批量转音频：狸窝：www.leawo.cn/（下载：视频转换器）

4. 免费搜电子书网站：https://www.jiumodiary.com

5. 语音转文字网站（迅捷转换器）https://app.xunjiepdf.com/voice2text/

6. 虚拟主播：http://anchor.avatarworks.com/#/news

7. Giios自动写作机器人：https://www.giiso.com/?t=2#/

8. 视频背景音乐自动匹配工具：https://cc.oceanengine.com/creative-factory/jishi/2

   

9. 图片微动工具：https://cc.oceanengine.com/creative-factory/ai/14

10. 网盘搜索工具：http://www.panduoduo.online/

11. 标题推荐工具：https://cc.oceanengine.com/creative-factory/miaobi

12. 抖音第三方数数据平台：https://www.chanmama.com/douyin?2004&360PC&PP

13. 新奇特好物产品渠道：https://www.lgn666.com/community/socialindex.html

14. 跨境电商导航：https://tools.ikjzd.com

15. 视频创作导航工具（工具大全）：https://www.aewz.com

16. 冷知识素材获取渠道 ：http://www.quzhishi.com/

17. 历史知识素材获取渠道：https://www.allhistory.com/book/home-list

18. 国学视频素材获取渠道：http://open.nlc.cn/onlineedu/course/explore/search.htm?categoryId=0&orderBy=latest

    

19. 高清无水印可商用在线免费下载图片素材获取渠道 ：https://www.pexels.com/zh-cn/

20. 高清无水印在线免费下载视频素材获取渠道：https://mazwai.com/

21. 图片版权查询工具：https://chacha.so.com/home?src=box

22. 一键背景抠图工具：https://www.remove.bg/zh

23. AI配音工具2：魔音工坊

24. 玄幻神话素材渠道：http://www.cbaigui.com/

25. 自动剪辑工具：http://www.animiz.cn/microvideo/?am

26. 星座素材获取网站：http://www.d1xz.net/

27. 美食类的账号素材网站：https://www.foodiesfeed.com/

28. Tik Tok卖家导航：https://www.tikmj.com/

    

29. 国外视频翻译网站：https://jianwai.youdao.com/

30. AI虚拟主播网站（腾讯出品）：https://zenvideo.qq.com/

31. 敏感词检测：http://www.ju1.cn/

32. 一键抠图网站（封面，换背景）：https://www.gaoding.com/koutu

33. 朋友防折叠网站：https://tingbiao.wang/tools/bzd/

34. 网盘搜索神器：https://www.dalipan.com/https://www.xiongbeng.com/

35. 聊天记录生成器：http://http://www.12tool.com/https://weixinduihua2.com/

36. 动画制作网站：https://www.laihua.com/

37. 带素材的网页剪辑工具：https://onvideo.kuaishou.com/

38. 网盘搜索聚合导航：https://www.chaonengso.com/

    

39. 商业案例素材网站：https://iwebad.com/

40. 国学视频素材网站：http://open.nlc.cn/onlineedu/client/index.htm

41. 图片加密工具：https://http://www.8tupian.com/

42. 抖音电商学习成长地图：https://fuwu.jinritemai.com/optimus/60ddc0d4b4558d003ffe5d8d

43. 历史故事网：https://http://www.88828.net/

44. 影视解说短视频，影视解说配音网站：http://www.6pian.cn/

45. 第三方数据平台推荐：蝉妈妈！新手机号注册可以免费领7天VIP，淘宝上卖的7天VIP，都是这样来的！

46. 设计用网站导航：http://http://www.46design.com/

47. 图片版权查询：https://chacha.so.com/home

48. 官方短视频批量剪辑生产工具：巨量创意——工具箱——微电影

    

49. 百度关键词规划（免费）https://fengchao.baidu.com/fc/manage/new/user/6808951/kr

50. 视频课程加密平台：https://cer.ieway.cn/

51. 一键抠图换背景：https://www.remove.bg/zh

52. 动图网站：https://gfycat.com/

53. 选题采集工具：https://www.houyicaiji.com/

54. 音频降噪：AU软件降噪

55. 录音软件：https://www.xunjieshipin.com/luyin

56. 根据台词找视频：电脑端——找台词（网页），手机端——33台词（APP）

57. 跨平台热点话题聚合网站：https://make.mk

58. 漫画版权申请入口（含文字教程）：http://www.kcbat.com/mhxpc.html

    

59. 免费通用AI配音网站：https://toolb.cn/textspeech

60. ‍表情包素材来源：http://www.dbbqb.com/

61. ‍语音克隆工具：https://github.com/babysor/MockingBird/blob/main/README-CN.md

    语音克隆教程：https://dpie96nlbr.feishu.cn/docx/doxcn7yE6JpLpJc3xVIrsBwyrEI

62. 抖音“前3秒”金句100例：https://school.jinritemai.com/doudian/web/article/aHWQ3MAaykVu

63. ‍视频漫画化：手机下载app：clip2comic，电脑网站：https://www.picup.shop/‍（皮卡智能，收费平台）

64. ‍根据关键词搜索视频素材：http://fse.agilestudio.cn/



# 木子琪

软件工具下载链接：

1：https://shimo.im/docs/Ee32mOodjQtPgvA2/ 

2：https://shimo.im/docs/Wr3DV9ogm8id1WkJ/ 

3：https://shimo.im/docs/PpQHvrDPvWW9Hvvq/ 

4：https://shimo.im/docs/CKTQ9vc6G8GkHgp3/

https://shimo.im/docs/1d3aM5B9MMU0Ll3g/

## 2024-05-08

**PixVerse**

PixVerse 是一个目前完全免费的 AI 一键生成视频在线网站。

它的界面是英文的，可以使用一些之前安利的网页翻译浏览器插件，例如：有道灵动翻译、火山翻译等，可以拥有更好的使用体验。

它上手容易，使用简单，第一次使用需要点击 Login with Email 按钮，用 QQ 邮箱注册一下，然后登录之后点击右上角创建按钮。

**它支持文字生成视频，也支持图片生成视频，其中文字生成视频需要输入文字描述，也可以输入不想呈现的内容，再选择视频风格和视频比例就可以一键生成视频，生成的视频可以下载保存。**

**Riffusion**

Riffusion 是一个免费的 AI 一键生成音乐在线网站。

它主要的特点是免费且无需注册登录，打开就可以使用；它支持输入的歌词字数有限制，主要用于生成简短的即兴重复段音乐。

**通义万相**

通义万相是一个免费且强大的 AI 一键生成图片在线网站。

它免费实用，功能非常强大，支持的功能包括：文本生成图像、相似图像生成、图像风格迁移、涂鸦作画、艺术字、虚拟模特等，每个功能都让人眼前一亮。

其中文本生成图像只需要输入文字描述、选择风格，就可以一键生成多张唯美图片，还支持上传参考图。

**在线图片处理工具合集**

在线图片处理工具合集是一个完全免费的在线图片处理网站。

它完全免费，不需要注册登录，打开就可以使用，支持多达 14 个实用图片处理工具，从上往下依次是：图片阴影消除、图片去除背景和抠图处理、图片无损放大、图片变暗、图片反转颜色、图片模糊处理、图片圆形裁剪、图片尺寸裁剪、图片上的颜色和色彩提取、图片变黑白、图片水平垂直翻转、图片变亮、图片彩色化和图片上色、图片旋转。

**在线图表制作**

快制图表是一个完全免费的在线图表制作网站。

它同样完全免费，无需注册登录，使用非常简单，颜值很高，支持 10 多种图表类型，包括：饼图、折线图、柱状图、雷达图、漏斗图、曲线图、散点图、蝴蝶图、横向柱状图、堆叠柱状图、横向堆叠柱状图；除了图表一键制作，它还支持一键生成多种图形素材，包括：圆形进度条、线性进度条、波浪进度条、仪表盘、圆形结构图、金字塔、维恩图。

**千峰办公助手**

千峰办公助手是一个完全免费的办公助手合集电脑软件，一个软件包含了几十个实用办公功能；软件支持 Windows 。

它完全免费，极简界面，一个软件相当于多个软件，支持的功能分类包括：PDF 工具、电子表格 Excel 工具、文字处理 Word 工具、OCR 图文识别工具、文件工具、图片工具、系统工具、键盘鼠标动作录制工具。

**其中 PDF 工具包括：提取图片扫描型 PDF 文字、PDF 转 Word 、PDF 转 Excel 、PDF 转 图片、PDF 转 TXT 、图片转 PDF 、提取 PDF 中表格、PDF 拆分、PDF 合并、PDF 嵌入图片、PDF 加密、PDF 添加水印。**

电子表格 Excel 工具包括：单表拆分为多表、多表合成为单表、单工作薄拆分为多工作薄、数据对比、数据提取；文字处理 Word 工具包括：Word 文档合并、Word 数据写入 Excel 、Word 表格导入 Excel 、Excel 数据写入 Word 。

**Excel工作圈小工具**

Excel工作圈小工具是一个完全免费的多功能 Excel 处理电脑软件；软件支持 Windows 。

它无需安装，双击打开就可以使用，体积小巧，只有 1M ，功能丰富，支持多达 50 多个 Excel 处理功能；它支持 Office 和 WPS ，支持的 Excel 处理功能分类包括：工作薄处理、工作表处理、单元格处理、文件处理、公式运用等。

其中工作薄处理包括：工作薄合并、工作薄所有公式转数值、工作薄浏览器、工作薄保护密码移除等，工作表处理包括：合并工作表、拆分工作表、拆分数据追加到分表、按列拆分为多个工作薄、创建工作表目录、批量修改工作表名称、工作表先后顺序调整、批量取消工作表隐藏、批量删除所有工作表图片对象。

**单元格处理包括：智能批量合并单元格、单元格数据补位、提取批注、文字型数字转数值型数字、数字转文字型数字、提取数字、提取字母、提取汉字、去字母、去汉字、去数字、合并单元格按类别求和、填充空白单元格、类别间隔填色。**

文件处理包括：批量修改文件后缀格式、提取文件夹中文件名称，公式运用包括：IF 公式合成、Vlookup 一对多查找。

**Pear-rec**

Pear-rec 是一个完全免费的多功能电脑软件，一个软件支持 10 多个实用功能；软件支持 Windows 。

它完全免费，简洁干净，所有功能一目了然，支持的功能包括：电脑录屏、电脑截图、电脑录制 GIF 、电脑贴图、电脑录像、查看图片、查看音频、查看视频、图片编辑、图片拼接、视频转 GIF 、以图搜图。

其中电脑贴图可以让图片钉在电脑屏幕上，电脑录制 GIF 不仅支持录制 GIF ，还支持编辑 GIF 。

**MindClear**

MindClear 是一个完全免费的电脑思维导图软件；软件支持 Windows 、macOS 。

它完全免费，支持多平台多端数据同步，画风唯美，颜值很高，使用简单，支持 2 种图形，包括：思维导图、组织架构图，支持 7 个主题，支持插入自由节点、关联线、摘要、图片、备注、超链接，支持丰富的设置选项。

**它支持新建文件夹和思维导图，支持设置思维导图背景色和彩虹分支线，支持多个节点设置**，包括：形状、填充颜色、边框粗细、边框颜色、分支线类型、分支线粗细、分支线颜色、文字大小、文字颜色、文字对齐、文字粗体、文字斜体、文字下划线、文字删除线。

它还支持多个关联线设置，全部设置完成，支持最高 5 倍 PNG 图片导出制作好的思维导图。

**eDiary**

eDiary 是一个免费好用的电脑本地日记笔记软件；软件支持 Windows 。

它已经持续更新了 20 多年，功能强大，使用场景丰富，包括：工作日志、学习心得、日记、笔记等。

**它支持全免费的纯本地使用，支持在没有网络的状态下使用，每个日记本对应一个单独的数据文件**，当然，它也支持多端数据云同步，这个功能需要高级版本才可以使用。

## 2024-05-09

**UVR5**

UVR5 是一个完全免费的人声伴奏分离电脑软件，也是一个特别好用的视频背景音乐提取软件；软件支持 Windows 、macOS 。

它完全免费，没有任何使用限制，不需要安装，是一个单文件，可以帮助你实现音乐人声伴奏分类和提取视频的背景音乐；它使用简单，实际效果特别出色，可以将人声和伴奏分离得非常干净清晰。

选择输入目录导入需要处理的音乐，选择输出目录设置导出音频文件的位置，支持 wav 、flac 、mp3 三种格式导出，支持多种处理方式和算法模型，我们选择默认的就行，也可以切换不同处理方式查看不同处理效果。

**可以勾选 GPU 转换就勾选它，处理速度会更快，不能勾选也没有关系，处理速度会稍微慢一点，不影响最后的处理效果**；点击开始处理按钮，会展示处理进度，稍微等待就可以看到分离的人声音频文件和伴奏音频文件。

**怡氧**

怡氧是一个完全免费的办公工具套件电脑软件；软件支持 Windows 、macOS 。

它完全免费，将思维导图、大纲笔记、流程图、Markdown 、Office 、PDF 标注集于一身，一个软件相当于多个软件，不需要网络，离线可用，不需要登录，极简干净，没有乱七八糟多余的东西，流畅舒适，非常好用。

它支持新建文件，包括：大纲笔记、流程图、思维导图、Markdown 、Word 、Excel 、PPT ，每新建一个文件就是一个标签，就像浏览器标签一样，可以任意切换，每个功能也提供了丰富的模板，在模板库中可以查看。

**LocalSend**

LocalSend 是一个完全免费的局域网文件传输软件；软件支持 Windows 、macOS 、Linux 、安卓、iPhone 、iPad 。

它小清新风格，画风唯美，支持跨平台传输，例如：电脑传输文件到手机、电脑传输文件到平板、 手机传输文件到电脑、手机传输文件到平板、安卓手机传输文件到苹果手机、苹果手机传输文件到安卓手机等。

它使用便捷，支持传输文件和文件夹，支持批量选择传输，可以一次性传输很多个文件。

**WinFR界面版**

WinFR界面版是一个完全免费的电脑文件恢复软件，可以帮助你恢复电脑中已经删除的文件；软件支持 Windows 。

它完全免费，极简界面，简单易用，可以帮助你恢复意外删除的文件，也可以从格式化硬盘中恢复文件，还可以从损坏的硬盘中恢复文件。

它支持选择丢失文件所在分区，也就是被删除文件的位置，支持选择恢复文件保存位置，支持深度扫描和快速扫描，深度扫描可以恢复更多已经丢失目录结构和文件名的文件。

**它支持恢复多种文件类型，包括：文档、图片、音频、视频、压缩文件，也支持指定恢复的文件类型**；它支持展示恢复文件进度和恢复文件的数量，恢复完成，所有恢复文件会按照不同文件类型分类。

**火山翻译**

火山翻译是一个完全免费的电脑浏览器翻译插件；软件支持 Windows 、macOS 。

它完全免费，功能丰富，支持的功能包括：网页全文翻译、划词翻译、划句翻译、划段翻译、输入翻译、PDF 划词翻译、单词本、自动朗读全文、语音朗读、吐司弹词、导出单词、个人术语，可以帮助你学习和办公。

**其中网页全文翻译支持双语对照和仅译文，支持选择切换多个不同领域翻译**，包括：办公协作、互联网行业、生物医疗等，可以获得更加精准的译文。

划词翻译支持添加至个人术语库，支持语音朗读，支持多个朗读声音任意切换。

**琅琅配音**

琅琅配音是一个完全免费的文字转语音和配音在线网站，手机平板和电脑都可以使用。

它完全免费，支持多种语言，包括：中文、英文、俄语、德语、法语、印尼语、意大利语、西班牙语、马来语、越南语、韩语、日语；它支持将近 100 种声音，中文声音分类包括：男声、女声、二次元、方言。

**它支持多人配音功能，这个功能可以实现每一个段落不同发音人，每一个句子不同发音人，每个字不同发音人。**

**AIDesign**

AIDesign 是腾讯出品的 Logo 一键生成在线网站。

它目前完全免费，是一个利用人工智能技术生成 logo 的智能设计平台，使用简单，让人惊艳。

输入名称，选择行业分类，挑选喜欢的关键词元素，选择配色，然后可以一键生成多个唯美 Logo ，部分 Logo 左上角会显示可以用于自身商业用途，界面右侧常见问题板块也对 Logo 的使用提供了详细的说明。

**历史年轮**

历史年轮是一个完全免费的历史学习在线网站。

它完全免费，颜值很高，支持的功能特点包括：用动态年龄和向量关系看历史、将古人代入到他的时代背景和周边人物中、数据可视化。

它提供了 3 万多个历史人物的详细介绍，按照不同朝代排列展示，支持历史人物分类功能，支持拖动年龄条展示历史事件，支持展示同时代相关历史人物、同时代热门历史人物、跨时代相关历史人物。

**爱发音**

爱发音是一个完全免费的英语音标和英语字母发音学习练习在线网站。

它极简设计，支持三个功能板块，包括：英式音标、美式音标、字母表，其中英式音标包括：长元音、短元音、双元音、清辅音、浊辅音。

**点击音标和字母可以发音，鼠标长按音标可以看到这个音标的视频学习**，非常直观，支持切换上一个下一个音标，可以帮助你更好的练习发音

**Simple mind map**

Simple mind map 是一个完全免费的电脑思维导图软件；软件支持 Windows 、macOS 。

它完全免费，支持便携版单文件版本，不需要安装，双击打开就可以使用，界面极简，功能全面，上手容易，使用简单。除了 8 种导图结构和将近 50 个思维导图主题任意切换，**它也支持高度自定义样式，包括：节点样式和基础样式**，具体设置选项包括：字体、字号、边框、背景、形状、线条、连线、关联线、关联线文字、水印等。

**每个节点支持普通文本和富文本两种类型，还支持添加图片、图标、贴纸、备注、公式、关联线、概要、标签、超链接。**

其中软件提供了丰富唯美的图标和贴纸，还有常用公式，支持输入 LaTeX 语法

**Talent**

Talent 是一个完全免费的浏览器多功能翻译插件；软件支持 Windows 、macOS 。

它完全免费，功能强大，流畅好用，支持的功能包括：多翻译源聚合翻译、专业划词翻译、划句翻译、划段翻译、鼠标悬停翻译、网页翻译、双语对照翻译、PDF 翻译、视频翻译、朗读助手等。

**划词翻译支持多个翻译引擎并同时展示多个翻译引擎结果**，支持语音朗读，支持鼠标停留在单词上自动查词、选中文字立即显示翻译结果、选中文字然后点击图标显示翻译结果、自动切分驼峰命名词组。

**爱思远控**

爱思远控是一个目前完全免费的远程控制软件；软件支持 Windows 、macOS 、安卓、iPhone 、iPad 。

它目前是完全免费的，支持 Windows 电脑之间远程控制，支持 macOS 电脑远程控制 Windows 电脑，支持 iPhone、iPad 远程控制 Windows 电脑，支持安卓、安卓平板远程控制 Windows 电脑。

它使用场景非常丰富，支持的功能和特点包括：**连接流畅稳定、毫秒级延时处理、多通道远程控制不再局限于一台主机、同时远控多台主机、对无人值守主机进行远程调试、多屏操作不限制展示被控多个屏幕、端端加密新一代加密框架保护数据安全、高清画质远程画面精细化处理**。

**VeryCapture**

VeryCapture 是一个完全免费的电脑截图录屏软件；软件支持 Windows 。

它完全免费，极简设计，界面好看简约，功能丰富，支持的功能包括：全屏截图、矩形截图、任意形状截图、延时截图、长截图、电脑录屏、电脑录制 GIF 、截图贴图、文字识别，一个软件相当于多个软件。

**其中截图之后还支持强大的截图标注处理功能**，包括：长方形、圆形、线段、箭头、画笔、马赛克、序号、添加文字等，截图之后也支持贴图功能和图片提取文字功能，**贴图功能可以将截图钉在电脑屏幕上**，让你更好的工作和学习。

**WeOCR**

WeOCR 是一个完全免费的可以离线使用的文字识别网站。

它完全免费，简洁干净，支持离线使用，有网络的状态下浏览器中打开它一次，然后就可以离线使用，之后没有网络也可以正常打开使用。

它主要功能是图片提取文字和文字 OCR 识别，支持粘贴图片和上传图片，支持复制识别的文字。

**LingoHut**

LingoHut 是一个完全免费的多语种学习网站。

它完全免费，一个网站相当于多个网站，可以学习多达 50 种语言，每种语言都提供了 125 节免费课程，每节课程可以学习多个单词和短语。

它支持学习单词短语、发音、释义，支持跟读，支持多个学习练习方式，包括：听力游戏练习、专注力游戏练习、闪卡翻转练习、单词配对练习等。

**识典古籍**

识典古籍是一个完全免费的古籍在线阅读学习网站。

它由北京大学和字节跳动数字人文开放实验室共建，目前收录了 2200 多部古籍，支持在线阅读，支持强大的学习功能。

它支持古籍分类功能，支持强大的古籍搜索功能。

**番茄AI配****音**

番茄AI配音是一个完全免费的文字转语音和配音微信小程序。

之前安利过几个免费的电脑文字转语音工具，完全免费的手机文字转语音工具比较少，而今天这款微信小程序，不仅可以在安卓手机上使用，也可以在苹果手机上使用。

**它完全免费，提供了几十种声音**，包括：新闻资讯、解说、方言等，每种声音包括多种情绪，例如：生气、抱怨、高兴、紧张、悲伤、严肃、沮丧，你可以选择一种声音和一种情绪进行配音。

## 2024-05-13

**有道灵动翻译**

有道灵动翻译是一个目前完全免费的浏览器翻译插件；软件支持 Windows 、macOS 。

它安装简单，功能实用，使用场景非常丰富，支持的功能包括：**一键翻译网页、输入框翻译**。

它支持选择有道翻译，支持选择 AI 大模型翻译，支持开启输入框翻译功能，支持仅展示译文，支持多种翻译样式，包括：下划线、颜色高亮、模糊模式。

其中仅展示译文功能就是只展示翻译后的文字内容，如果没有勾选这个选项，原文和译文都会展示，并且译文会展示在原文下方；模糊模式当鼠标放上去会展示译文，当鼠标移开就模糊译文。

**一点红语音合成**

一点红语音合成是一个完全免费的文字转语音电脑软件；软件支持 Windows 。

它完全免费，无需注册登录，极简设计，没有任何乱七八糟多余的东西，非常干净，上手容易，使用简单。

它没有字数限制，几万个字生成语音也是非常轻松，文字转语音速度非常快，支持几十个语音任意切换，也可以在人物板块试听不同语音。

它支持设置生成音频的风格强度、语速、音调，支持选择音频格式 MP3 和 WAV ，点击音频生成按钮就可以开始转换，转换完成，点击音频试听按钮就可以播放生成的音频，点击音频文件按钮可以打开音频文件的位置。

**批量加水印**

批量加水印是一个完全免费的图片批量添加水印电脑软件；软件支持 Windows 。

它完全免费，无需安装，双击打开就可以使用，**不需要网络，支持离线使用**，体积小巧，不到 1M 。

它支持批量图片添加水印，可以给很多张图片添加一个水印，将所有需要处理的图片放到同一个文件夹中，然后将这个文件夹拖入到软件的图片文件框中。

**它支持多个水印设置选项，包括：文字内容、字体、字体大小、字体颜色、不透明度、横向水印个数、纵向水印个数、水印旋转角度等，设置完成，点击开始合并按钮即可。**

它支持双击设置水印字体和水印字体颜色，它的图片添加水印速度非常快。

**云展网办公工具集**

云展网办公工具集是云展网旗下的办公小工具集合网站，让人惊艳。

它完全免费，所有功能都是免费使用，没有什么限制，非常干净，没有水印，功能强大，一个网站支持几十个实用功能，并且一直在增加功能。

**它所有上传的文件和结果文件都会在 6 小时后自动删除，保护你的文件安全；它所有功能和工具几乎都支持批量处理文件。**

它的使用场景非常丰富，支持手机、电脑、平板打开使用。

**AI人像生成多风格漫画**

AI人像生成多风格漫画是一个完全免费的图片风格卡通化在线网站。

它完全免费，简单干净，无需注册登录，打开网站就可以使用，不需要任何多余的操作，可以将你的图片一键变为卡通风格、漫画风格。

**它支持 5 个图片风格和效果，包括：漫画风格、3D 卡通风格、手绘风格、素描风格、艺术风格，每种风格处理效果都是非常惊艳。**

转换完成，点击右上角下载按钮可以将生成的不同风格图片下载下来。

**FOCA**

FOCA 是一个免费图片、视频、设计模板素材网站。

它免费的图片、视频和设计模板素材可以用于个人和商业用途，免费可商用；它界面清新优雅，非常干净，简洁唯美。

**它的图片、视频和设计模板素材的质量非常高，使用场景丰富，支持强大的分类功能和搜索功能；其中图片素材支持高清大图下载，视频素材也是高分辨率视频素材。**

最后，它还一直在增加新的内容和文件素材。

**PixPin**

PixPin 是一个完全免费的电脑截图软件和多功能办公软件；软件支持 Windows 。

它目前完全免费，极简干净，非常轻量级，使用起来流畅舒适，功能丰富，支持的功能包括：电脑截图、区域识别截图、截长图、贴图、贴图后继续截图、GIF 录制、离线 OCR 文字识别、截图标注编辑、颜色拾取。

**它上手容易，使用非常简单，默认快捷键 Ctrl+1 就可以开始截图，鼠标移动就可以看到颜色拾取功能，可以帮助你提取电脑屏幕的任意颜色。**

它支持区域识别截图功能，将鼠标移动到某个区域，支持自动识别这个区域，然后可以实现快捷截图。

**它支持电脑长截图功能，支持横向长截图，也支持纵向长截图。**

**PDFgear**

PDFgear 是一个完全免费的电脑 PDF 编辑处理和 PDF 转换软件；软件支持 Windows 、macOS 。

它良心好用，无需注册登录，简约设计，功能全面丰富，点击打开文件按钮就可以导入 PDF 文件。

它支持 PDF 批注功能，包括：文本高亮、区域高亮、删除线、下划线、矩形、直线、椭圆、画笔、插入文字、文本框、便签、图章，支持设置颜色、线条粗细、文字大小等。

它支持编辑 PDF 文字内容，支持合并拆分 PDF ，支持提取删除 PDF 页面，还支持添加图片、插入页面、裁剪页面、设置密码、清除密码、新建水印、删除水印、页码、页眉页脚、压缩。

**其中 PDF 压缩、PDF 去除水印、PDF 清除密码、PDF 编辑文字内容、PDF OCR 提取文字内容、PDF 合并拆分都是非常实用。**

除了强大的 PDF 编辑功能，它也支持 PDF 阅读功能，支持多个 PDF 阅读选项。

**闪电藤**

闪电藤是一个完全免费的局域网文件传输软件；软件支持 Windows 、MacOS 、Linux、安卓、iOS 。

它完全免费，简洁干净，如一股清流，主要特点包括：无需联网、无需登录、不限设备、没有文件大小的限制、多端文件互传、不限速、传输速度极快、安全稳定。

它使用非常简单，打开软件就知道怎么操作，支持发送文字消息、图片文件、视频文件、文档文件、压缩包文件、文件夹，还支持剪贴板同步。

**TME Studio**

TME Studio 是腾讯音乐推出的一个音乐分离网站和在线音乐创作助手工具。

它免费好用，支持的功能包括：音乐分离、计算音频信息、辅助写词、智能曲谱，每个功能都是非常好用，效果出色。

**其中音乐分离功能特别实用，不仅可以帮助你分离人声，还可以分离音乐的钢琴声、吉他声、贝斯声、鼓声、其它声音，每个声音单独一个轨道，支持只播放一个轨道的声音，支持提取这个轨道的声音，支持提取所有轨道的声音。**

也就是说，这个功能可以帮助你分离人声和其它声音，导出下载分离的声音。

**TTS-Online**

TTS-Online 是一个完全免费的文字转语音在线网站。

它完全免费，一目了然，上手容易，使用简单，内置了 160 多种声音，非常真实自然，富有情感，适合多个场景，包括：解说、方言、外语、二次元等。

**它支持直接试听声音，其中外语声音包括：英文、俄语、法语、德语、意大利语、西班牙语、葡萄牙语、韩语、日语。**

## 2024-05-16

**思绪思维导图**

思绪思维导图是一个完全免费的在线思维导图网站。

它完全免费，不需要注册和登录，画风好看，支持将近 50 个思维导图主题任意切换，支持 8 种思维导图结构任意选择。

它功能全面，支持的功能包括：提供精美的图标素材和贴纸素材、添加图片、添加备注、添加公式、提供多个常用公式、添加关联线、编辑思维导图大纲、设置节点样式、设置基础样式。

**它支持多达 7 种格式导出制作好的思维导图，包括：PDF 、PNG 、SVG 、Markdown、JSON 等。**

它在安卓手机、苹果手机、安卓平板、iPad 上同样拥有不错的使用体验。



**咔咔一通剪**

咔咔一通剪是一个功能强大的电脑视频批量处理软件；软件支持 Windows 。

它完全免费，打开一目了然，每个功能板块非常直观，支持批量导入视频和批量处理视频，可以一次性处理很多个视频。

**它支持的视频批量处理功能包括：视频合并、视频分割、视频格式转换、视频压缩、视频添加水印、视频去除水印、视频去除音频、视频调整音量、视频增加背景音乐、修改视频分辨率、视频横竖屏转换、视频添加片头片尾、视频去除片头片尾、视频提取图片、视频转图片；其中视频合并、分割、提取图片都支持多种模式。**

还支持视频变速、阴影、对比度、视频模糊、锐化、色彩饱和度、旋转、色调、光晕效果、亮度调整等。

 

**法行宝**

法行宝是百度出品的免费 AI 律师在线网站。

它完全免费，界面简洁干净，不需要注册和登录，打开就可以使用，是一个免费智能律师助手。

它简单快捷，有专业法律问题可以随时问，支持法律咨询、解答法律问题、学习法律知识、给予专业法律建议，还支持生成定制化和精准详细的法律咨询意见书。



**Okular**

Okular 是一个免费好用的电脑文档阅读编辑软件；软件支持 Windows 、Linux 。

它完全免费，支持的文档格式包括：PDF 、EPub 、DjVU 、MD 、Tiff 、 WebP 、CBR 、CBZ 、CHM 、XPS 等。

**它启动速度很快，使用流畅，对几百 M 大体积文档的打开速度也是非常迅速，支持多个文档标注编辑功能，包括：高亮、直线下划线、波浪下划线、删除线、文字批注、文本框批注、手绘线条、箭头、直线、矩形、椭圆形、多边形、图章。**

它还支持多种视图、批注工具栏、目录栏、选择区域、选择文字、选择表格、放大镜模式。

 

**证件照大师**

证件照大师是一个目前完全免费的证件照制作在线网站。

它免费好用，支持的功能包括：证件照换底色、PNG 证件照制作、证件照剪切合成、将证件照压缩到指定的体积和大小、证件照剪切、证件照打印排版。

它便捷高效，每个功能都提供了详细的使用方法，其中证件照打印排版支持设置证件照尺寸、相纸尺寸、相纸方向、输出格式、页边距、分隔像素。



**Pot**

Pot 是一个电脑免费聚合翻译软件；软件支持 Windows 、macOS 、Linux 。

它小清新风格，颜值很高，支持的功能包括：划词翻译、输入翻译、文字识别、截图翻译、语音朗读翻译结果、离线识别、生词本；它聚合了多个翻译引擎和文字识别服务，支持同时展示多个翻译引擎的翻译结果。

**它还支持插件功能，可以帮助你更好的使用这款软件**，还支持翻译历史记录功能；如果第一次打开软件是英文界面，找到 General 按钮，再点击 Application Language 按钮就可以切换为中文界面。

 

**SimpleTex**

SimpleTex 是一个完全免费的公式识别编辑电脑软件；软件支持 Windows 、macOS 。

它良心免费，支持上传公式图片、识别公式、编辑公式，支持 80 多种语言识别，支持各种复杂公式和化学结构式，包括：手写公式、打印公式、手写化学结构式、打印化学结构式、多行复杂公式、文本公式混排、图表多栏排版论文等。

**它识别速度很快，识别非常准确，支持复制 LaTeX 、复制 MathML 、复制 SVG 到 Word 和 WPS 软件中。**

它还支持公式识别记录、文档编辑器等功能；它可以实现快速识别公式，对写论文非常有帮助。



**土豆录屏**

土豆录屏是一个免费强大的电脑录屏软件；软件支持 Windows 。

它完全免费，简单易用，简洁纯净，功能丰富，最高支持录制 4K 和 120FPS 画质视频，支持设置录制文字水印、支持录屏时显示摄像头画面，支持录制时高亮鼠标和鼠标光圈显示，支持计划定时录制和录制完成电脑关机。

它还支持的设置选项包括：全屏录制、区域录制、应用窗口录制、录制麦克风、录制系统声音、录制麦克风和系统声音、录制清晰度和画质、分段保存录制文件、录制快捷键。

 

**PDF转换器**

PDF转换器是一个免费强大的 PDF 转换网站。

它简洁干净，无需注册登录，完全免费，打开就可以上传文件转换处理文件，支持的功能包括：PDF 转 Word 、PDF 转 PPT 、PDF 转 Excel 、Word 转 PDF 、Word 转 PDF 、Excel 转 PDF 、PDF 转 Markdown 、Word 转 Markdown 、Excel 转 Markdown 、PDF 转 TXT 、Word 转 TXT 。

它支持将 PDF 文件转换为可编辑的 Word、PPT 、Excel 文件，最大支持 50M ，转换速度快，转换效果不错。



**一念桌面**

一念桌面是一个完全免费的电脑视频桌面壁纸软件；软件支持 Windows 。

它非常纯粹，主要功能就是将电脑本地视频设置为电脑桌面壁纸，让你的电脑桌面壁纸变得动态唯美；它支持设置视频壁纸的音量，支持开启桌面动态时钟，支持手动添加视频素材和切换视频壁纸，还支持定时更换壁纸功能，可以在配置文件中修改壁纸更换时长。



**Audio Trimmer**

Audio Trimmer 是一个完全免费的在线音频编辑处理网站。

它完全免费，一个网站支持多个功能，包括：音频剪辑器、MP3 转换器、音频变速、音频倒放、音量增强、背景音乐素材合集。

其中音频剪辑功能最大支持 250M ，MP3 转换器支持将其它格式音频和视频转换为 MP3 格式音频，支持将近 20 种格式；背景音乐素材合集提供了 30 多首高品质悦耳背景音乐素材，并且这些音频素材是免版税的。



**免费在线抠图**

免费在线抠图是一个完全免费的在线抠图网站。

它良心免费，使用场景丰富，不需要注册登录，支持上传和拖入图片，支持输入图片链接，图片大小最大 5M 。

它不会存储任何图片数据，抠图速度很快，抠图效果特别出色，支持抠图前后对比，支持下载抠图后的图片文件。



**热速美斯**

热速美斯是一个免费实用的简历制作网站。

它完全免费，支持 12 个简历模板，每个简历模板都是简约大气，清新唯美；它支持丰富的简历编辑功能，包括：文本、图标、形状、配色、空白填充、页面背景色、圆角，还支持上传图片、添加新页面、复制页面、拆分组合、锁定组合、复制组合等。

全部设置完成，它支持 PDF 格式导出制作好的简历。



**水滴英语**

水滴英语是一个免费英语学习和英语作文在线网站。

它支持的功能包括：英语美文、英语句子、英语作文万能句子、自我介绍英语作文、关于秋天的作文、关于动物的作文、关于诚信的作文、大学英语作文、初中高中英语作文等，可以帮助你提高英语作文水平。



**Ugly Avatar**

Ugly Avatar 是一个有趣好玩的丑头像生成器网站。

它完全免费，无需注册登录，非常干净，打开就知道怎么使用，可以帮助你一键生成很多个搞怪个性的头像，支持发现新的头像，支持 PNG 格式下载生成的头像。

## 2024-07-11

**口语猫**

口语猫是一个目前完全免费的英语口语 AI 训练软件；软件支持安卓。

它目前是完全免费的，支持的功能包括：雅思陪练、AI 虚拟教师陪伴式学习、AI 发音模拟日常人际交流对话、引导式提问、AI 智能打分。

它支持情景对话功能，包括：个人相关、日常生活、兴趣爱好、社交活动等，其中日常生活包括：订机票、邮寄快递、问路、家庭生活、谈论宠物、公共交通、美食和烹饪。

它支持选择 AI 教练对话，支持展示对话翻译，支持听力训练和口语训练。



